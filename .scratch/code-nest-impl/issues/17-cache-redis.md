...
- **接入**：
  ...
  - 消费者 `ArticleCacheEvictListener` 消费 `article.cache-evict`（绑定 `article.updated`、`article.deleted`），再调用一次 `evictCache`。发布不做第二次删除。删除本身幂等，所以不加 `@IdempotentConsumer`；两档都装配，`none` 档下什么也不做。
