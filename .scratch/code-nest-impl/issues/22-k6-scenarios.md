...
- **场景取值**：A 用 `user_20000` 起的 200 个账号（不写文章），点赞最新发布的一篇文章；A 压测后等 10 秒再采集并断言 `article_stat.like_count` 等于点赞行数，不等就退出。B 的 200 个 VU 轮流使用 100 个重度用户。D 的候选是最新发布的 1000 篇，Zipf 指数 1.2，前 10 篇约占 57%。
- **推送耗时**：推送会跳过收件箱不存在的粉丝，造数后收件箱都不存在，所以 prepare 阶段让 author_4999 的 4999 个粉丝各读一次 Feed（约 2 分钟）。推送按粉丝 ID 升序，轮询 ID 最大的粉丝的 Feed 直到出现新文章，测完删文。只在 push-pull 档测。
