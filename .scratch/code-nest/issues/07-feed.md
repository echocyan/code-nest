...
1. **策略**：
   ...
   - 每个作者都有一个**发件箱**，保存最近 100 篇文章，有三个用途：读 Feed 时拉取大 V 的文章；新关注某人时补齐他的文章；重建冷用户的收件箱。发件箱由 social 模块消费 `article.published` 维护，这样 social 不需要反复调用 `ArticleApi` 查询。应用启动时如果重建完成标记 `feed:outbox:ready` 不存在（首次部署、Redis 数据丢失、造数绕过发文事件直接写库），就经 `ArticleApi` 按文章 ID 正序遍历全部已发布文章重建发件箱，完成后写入标记。
2. **冷用户**：
