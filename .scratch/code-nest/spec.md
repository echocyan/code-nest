...
83. 作为运维者，我希望 Redis 数据丢失后，计数能从 MySQL 懒加载恢复，布隆过滤器和 Feed 发件箱在启动时重建，Feed 收件箱在首次读取时重建，这样缓存层是可重建的。
...
- **维度**：需要登录的接口按用户 ID；匿名接口按 IP，只对可信代理解析 `X-Forwarded-For`，从右往左取第一个不是可信代理的地址。
- **声明方式**：可重复标注的 `@RateLimit(key, limit, window, dimension)`，由注册在 `SaInterceptor` 之后的
  `HandlerInterceptor` 执行。
- **默认限额**：见用户故事 71–73，全部在 `rate-limit.limits` 下配置。发文按发布接口计数；评论与回复、点赞与收藏（含取消）、
  关注与取关各共用一份额度。一次请求要所有额度都有余量才放行，被拒绝的请求不占额度。
- **开关**：`rate-limit.enabled`。
- **响应与降级**：超限返回 429、错误码 90429，并带 `Retry-After`（等到腾出名额的秒数，向上取整）；Redis 故障时放行（fail-open）。
...
- **造数**：loadtest 模块里的 JDBC 造数程序，使用固定随机种子。
    ...
    - 派生数据走系统自带的重建路径生成：计数执行对账，ES 调用重建端点，布隆过滤器和 Feed 发件箱在启动时重建，收件箱在首次读取时懒重建。
- **场景**：
    - A 计数：热门文章被集中点赞。
...
- **只有以下几类行为在各自的公开接口上单独测**，因为从 HTTP 层看不到：
    ...
    4. **纯逻辑单元测试**（普通 JUnit）：热度公式。
    5. **滑动窗口的时间边界**：逻辑都在 Lua 脚本里，直接调用限流器对真实 Redis 测。
    6. **ArchUnit**：跨模块只能调用 `api` 包，模块依赖无环。
